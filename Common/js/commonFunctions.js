...
//======================================================================================================================//
function refreshdata(tabToBeDisplay,forceGetParam){
	var forceGet = false;
	if(!IsValueNull(forceGetParam)){	// If set to true, Reloads the current page from the server
		forceGet = forceGetParam;
	}
	var date = new Date();
...
{
	var callType = false, sendMethod	=	'POST', additionalData	=	null, callBack	=	'', responseObj	= 0;
	...
	if(!IsValueNull(object.callBack) )
	{
		...
		if(!IsValueNull(object.responseType) )
		{
			responseObj	=	object.responseType;
		}
		xmlHttp.onreadystatechange	=	function(){
			if(xmlHttp.readyState	==	4 && xmlHttp.status	==	200){
				if(responseObj == 1){
...
// function to create date object from a date time string of format yyyy-mm-dd HH:MM:SS
// for example createDateFromString(2015-06-18 05:04:06, -12)
function createDateFromString(dateTimeString, timezoneOffsetHours){
	if(IsValueNull(timezoneOffsetHours)){
	...
	MM		=	timeString[1];
	SS		=	timeString[2];
	var originalDate	=	new Date(yyyy, parseInt(mm)-1, dd, HH, MM, SS);
	newDate	=	originalDate.getTime()+(timezoneOffsetHours*60*60*1000);
	return new Date(newDate);
}

// function to compare date from two date time strings of format yyyy-mm-dd HH:MM:SS
// for example compareDateOnly(2015-06-18 05:04:06, 2015-06-25 05:04:39)
// return true if Date1 is more than equal to Date2
function compareDateOnly(Date1, Date2, offsetHour1, offsetHour2){
	if( (typeof(Date1)== 'string') && Date1.indexOf(' ') != -1){
		...
		Date1	=	new Array(Date1);
	}
	if( (typeof(Date2)== 'string') && Date2.indexOf(' ') != -1){
		Date2	=	Date2.split(' ');
	}
	else{
		Date2	=	new Date(Date2);
		Date2	=	Date2.getFullYear()+'-'+(Date2.getMonth()+1)+'-'+Date2.getDate(); 
		Date2	=	new Array(Date2);
	}
	return (createDateFromString(Date1[0]+' 00:00:00',offsetHour1) >= createDateFromString(Date2[0]+' 00:00:00',offsetHour2));
}

//function to compare time from two date time strings of format yyyy-mm-dd HH:MM:SS
//for example compareTimeOnly(2015-06-18 05:04:06, 2015-06-25 05:44:06)
//return true if Date1 is more than Date2
function compareTimeOnly(TDate1, TDate2, ToffsetHour1, ToffsetHour2){
	if( (typeof(TDate1)== 'string') && TDate1.indexOf(' ') != -1){
		TDate1	=	TDate1.split(' ');
	}
	else{
		TDate1	=	new Date(TDate1);
		TDate1	=	TDate1.getHours()+':'+TDate1.getMinutes()+':'+TDate1.getSeconds();
		TDate1	=	['',TDate1];
	}
	if( (typeof(TDate2)== 'string') && TDate2.indexOf(' ') != -1){
		TDate2	=	TDate2.split(' ');
	}
	else{
		TDate2	=	new Date(TDate2);
		TDate2	=	TDate2.getHours()+':'+TDate2.getMinutes()+':'+TDate2.getSeconds(); 
		TDate2	=	['',TDate2];
	}
	return (createDateFromString('2015-05-25 '+TDate1[1],ToffsetHour1) >= createDateFromString('2015-05-25 '+TDate2[1],ToffsetHour2));
}